...
int get_student(int fd, int id, student_t *s)
{
    ...
    // Parse through file until student is found
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // Check if student already exists; handle error if student does exist
    if (get_student(fd, id, &s) == NO_ERROR) {
        printf(M_ERR_DB_ADD_DUP, s.id);
        return ERR_DB_OP;
    } 

    // Check if memory location is empty, otherwise handle error if student already exists
    if (memcmp(&s, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0) {
        printf(M_ERR_DB_ADD_DUP, s.id);
        return ERR_DB_OP;
    }

    // Ensure file is large enough; handle write file error if unable to
    if (ftruncate(fd, offset + sizeof(student_t)) == -1) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }

    // Add values to new_student record
    new_student.id = id;
    ...
    // Seek correct memory location; handle write file error if unable to
    if (lseek(fd, offset, SEEK_SET) == -1) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }

    // Write new student record to file; handle write file error if unable to
    bytes_written = write(fd, &new_student, sizeof(student_t));
...
int del_student(int fd, int id)
{
    ...
    // Check if student exists; handle error if student does not exist
    if (get_student(fd, id, &s) == SRCH_NOT_FOUND) {
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    } 

    // Seek correct memory location; handle read file error if unable to
    if ((lseek(fd, offset, SEEK_SET)) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    } 
    
    // Delete student record by overwriting with EMPTY_STUDENT_RECORD
...
int count_db_records(int fd)
{
    ...
    // Start from beginning of the file; handle error if unable to read file
    if (lseek(fd, 0, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    // Read records and count non-empty records
    while ((bytes_read = read(fd, &student, sizeof(student_t))) == sizeof(student_t)) {
...
int print_db(int fd)
{
    ...
    while ((bytes_read = read(fd, &student, sizeof(student_t))) == sizeof(student_t)) {
        ...
        // Print header once if database is not empty
        if (!non_empty_db) {
...
{
    // TODO
    // Handle error if student pointer is null or student ID is 0
    if ((s == NULL) && (s->id != 0)) {
...
int compress_db(int fd)
{
    ...
    // Open temporary database; handle error if unable to open database file
    tmp_fd = open(TMP_DB_FILE, O_WRONLY | O_CREAT | O_TRUNC , S_IRUSR | S_IWUSR);
    if (tmp_fd == -1) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }

    // Read & copy records from original file to temp file
    while (true) {
        // Seek memory location; handle error
        if (lseek(fd, offset, SEEK_SET) == -1) {
            printf(M_ERR_DB_READ);
            close(tmp_fd);
            return ERR_DB_FILE;
        ...
        if (memcmp(&s, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            ssize_t bytes_written = write(tmp_fd, &s, sizeof(student_t));
            // Write non-empty student record to temporary file
            if (bytes_written != sizeof(student_t)) {
                printf(M_ERR_DB_WRITE);
                close(tmp_fd);
                return ERR_DB_FILE;
    ...
    // Reopen compressed database file
